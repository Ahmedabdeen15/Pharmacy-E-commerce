...
import 'package:pharmaco/view/screen/account_password/reset_passwrd_third_screen.dart';
import 'package:pharmaco/view/widget/custom_app_bar.dart';
import 'package:pharmaco/view/widget/generic_flexible_button.dart';
import 'package:pharmaco/view/widget/input_verification_code_widget.dart';

class ResetPasswordSecoundScreen extends StatelessWidget {
  const ResetPasswordSecoundScreen({super.key});
      ...
      appBar: CustomAppBarr(title: "Reset Password"),
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 10,),
            Container(alignment:Alignment.centerLeft,padding:EdgeInsets.only(left:  MediaQuery.of(context).size.width*.1),child:  Text("Enter Code",style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold),)),
            SizedBox(height: 10,),
            InputVerificationCodeWidget(inputHint: "**********"),
            SizedBox(height: 20,),
            GenericFlexibleButton(text: "Continue", minWidth: MediaQuery.of(context).size.width*.8, minHeight: MediaQuery.of(context).size.height*.09, fontSize: 24, onPressed: (){
                Navigator.push(context, MaterialPageRoute(builder: (context) => const ResetPasswrdThirdScreen() ));
            })
